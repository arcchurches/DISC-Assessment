...
if($_GET['getResults'] == 1) {
	...
	foreach($sres as $key=>$val) {
		$result .= "<h3>".$descriptions['sg'.$key]."</h3>";
		$result .= "<p>".$descriptions['sgd'.$key]."</p>";
		$result .= "<hr/>";
	}
}
    ...
    $disc_results['type'] = ucwords($aks[0]) . '/' . ucwords($aks[1]);
    $disc_results['errors'] = $errors;
    $disc_results['description1'] = $descriptions[$aks[0].$aks[1]];
    $disc_results['description2'] = $descriptions['byg'.$aks[0]];
    $disc_results['gifts'] = $sres;
    //return $disc_results;
    echo $result;
    $_SESSION['disc_results'] = $disc_results;
    $_SESSION['disp_results'] = $result;
?>
